...
// OPTIMIZATION: Index schedules by train_no for O(1) lookup
// This prevents O(N*M) filtering inside the simulation loop
...
const generateLiveStatus = (now: Date = new Date()): TrainStatusItem[] => {
  ...
  TRAINS_DATA.forEach((train) => {
    ...
    // OPTIMIZATION: Use O(1) Map lookup instead of O(N) filter
