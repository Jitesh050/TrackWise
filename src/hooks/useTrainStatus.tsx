...
// Pre-group schedules by train number for O(1) access
// Optimized: Replaced repeated array filtering (O(N*M)) with Hash Map lookup (O(N))
const SCHEDULES_BY_TRAIN: Map<string, ScheduleRecord[]> = new Map()
...
const generateLiveStatus = (now: Date = new Date()): TrainStatusItem[] => {
  ...
  TRAINS_DATA.forEach((train) => {
    ...
    // Optimized: Use Map lookup instead of .filter()
