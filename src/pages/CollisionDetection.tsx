...
// Pre-compute the schedules grouping outside the component to avoid O(N) reallocation
// and heavy processing on every render.
const SCHEDULES_BY_TRAIN: Record<string, any[]> = {};
(schedules as any[]).forEach((s) => {
  (SCHEDULES_BY_TRAIN[s.train_no] ||= []).push(s);
});

// Pre-sort the arrays so we don't need to do it during average speed calculation
...
const CollisionDetection = () => {
  ...
  const { totalTrains, activeStations, onTimePct, averageSpeed } = useMemo(() => {
    ...
    list.forEach((t: any) => {
      ...
      if (String(t.status).toLowerCase().includes("on time")) onTimeCount++;
