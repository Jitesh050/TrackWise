...
// ⚡ Bolt Optimization: Hoist static schedule grouping and sorting outside the component
// to prevent O(M) allocations and O(K log K) sorting per render.
const STATIC_SCHEDULES_BY_TRAIN = (() => {
  const byTrain: Record<string, any[]> = {};
  (schedules as any[]).forEach((s) => {
    (byTrain[s.train_no] ||= []).push(s);
  });

  // Pre-sort schedules for each train once
...
const CollisionDetection = () => {
  ...
  // ⚡ Bolt Optimization: Consolidate multiple O(N) train array passes into a single pass memoized block.
  const { totalTrains, activeStations, onTimePct, averageSpeed } = useMemo(() => {
    ...
    list.forEach((t: any) => {
      ...
      // 2. Active Stations check
      if (t.nextStation) {
        stationsSet.add(t.nextStation);
      }

      // 3. Average Speed calculation (using pre-grouped and pre-sorted static data)
