...
// Hoist static schedules grouping outside of the component to prevent expensive O(N) execution on every render
...
const CollisionDetection = () => {
  ...
  // Consolidate multiple O(N) array traversals (totalTrains, activeStations, onTimePct, averageSpeed) into a single pass wrapped in useMemo
  const { totalTrains, activeStations, onTimePct, averageSpeed } = useMemo(() => {
    const list = trains || [];
    const len = list.length;
    if (len === 0) {
      return { totalTrains: 0, activeStations: 0, onTimePct: 0, averageSpeed: 0 };
    }
    ...
    list.forEach((t: any) => {
      if (t.nextStation) {
        uniqueStations.add(t.nextStation);
      }
      ...
      if (arr.length >= 2) {
        // Assume sorting was done beforehand or is fast enough for small arrays
        const sortedArr = [...arr].sort((a, b) => (a.day_offset - b.day_offset) || (a.seq - b.seq));
    ...
    return {
      totalTrains: len,
      activeStations: uniqueStations.size,
