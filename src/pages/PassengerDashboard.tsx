...
const PassengerDashboard = () => {
  ...
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    const hasBookings = ticketsList.length > 0;
    const latestPnr = hasBookings ? (ticketsList[0]?.pnr || "") : "";
    const now = Date.now();
    ...
    for (const t of ticketsList) {
      ...
      if (!isNaN(time) && time >= now && time < earliestUpcomingTime) {
        earliestUpcomingTime = time;
      }
    }

    const nextJourney = earliestUpcomingTime !== Infinity
      ? new Date(earliestUpcomingTime).toLocaleString()
      : "No upcoming";
