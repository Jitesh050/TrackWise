  ...
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    for (const t of safeTickets) {
      if (t.status === "Confirmed") {
        ...
        const fromStation = STATIONS_BY_CODE.get(fromCode);
        const toStation = STATIONS_BY_CODE.get(toCode);
    ...
    return {
      ...
      hasBookings,
      latestPnr,
      nextJourney: closestTime === Infinity ? "No upcoming" : new Date(closestTime).toLocaleString(),
