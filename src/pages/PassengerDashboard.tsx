...
// Build quick lookup of station by code outside component to avoid reallocation
...
const PassengerDashboard = () => {
  ...
  // Consolidate multiple O(N) array passes into a single loop wrapped in useMemo
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    let active = 0;
    let kmTotal = 0;
    let upcomingTime = Infinity;
    const now = Date.now();
    const len = tickets?.length || 0;
    const latest = len > 0 ? (tickets[0]?.pnr || "") : "";
    ...
    const nextJStr = upcomingTime !== Infinity ? new Date(upcomingTime).toLocaleString() : "No upcoming";
    return {
      activeBookings: active,
      hasBookings: len > 0,
      latestPnr: latest,
      nextJourney: nextJStr,
