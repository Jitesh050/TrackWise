...
const PassengerDashboard = () => {
  ...
  // ⚡ Bolt Optimization: Consolidate multiple O(N) ticket array iterations into a single pass
  // and cache the result using useMemo to prevent unnecessary calculations on every re-render.
  // Performance impact: O(N * operations) reduced to O(N).
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    const upcomingJourneys: { time: number }[] = [];
