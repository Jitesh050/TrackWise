...
// ⚡ Bolt: Cache station lookup map outside component to prevent O(N) reallocation per render
...
const PassengerDashboard = () => {
  ...
  // ⚡ Bolt: Consolidate multiple O(N) filtering, mapping, reducing, and sorting passes
  // into a single O(N) pass wrapped in useMemo. Reduces redundant calculations per render.
  const {
    activeBookings,
    hasBookings,
    latestPnr,
    ...
    for (const t of tList) {
      ...
      // ⚡ Bolt: Replace O(N log N) sorting with an O(N) minimum-finding approach
      const timeStr = t.date + (t.departureTime ? " " + t.departureTime : "");
      const tTime = Date.parse(timeStr) || Date.parse(t.date);
      if (!isNaN(tTime) && tTime >= now && tTime < nextTime) {
        nextTime = tTime;
      }
    }

    const has = tList.length > 0;
    return {
      activeBookings: active,
      hasBookings: has,
      latestPnr: has ? (tList[0]?.pnr || "") : "",
      nextJourney: nextTime !== Infinity ? new Date(nextTime).toLocaleString() : "No upcoming",
