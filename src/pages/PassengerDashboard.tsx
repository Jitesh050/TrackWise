...
const PassengerDashboard = () => {
  ...
  // ⚡ Bolt: Optimized performance by wrapping multiple O(N) array transformations
  // (filter, map, reduce, sort) into a single-pass O(N) loop memoized by useMemo.
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    let active = 0;
    let has = false;
    let latest = "";
    let minTime = Infinity;
    let nextDate = "No upcoming";
    let kmTotal = 0;

    const byCode = STATIONS_BY_CODE;
    ...
    if (minTime !== Infinity) {
      nextDate = new Date(minTime).toLocaleString();
    }
