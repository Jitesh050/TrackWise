...
// Build quick lookup of station by code once at module level to avoid O(N) reallocation on render
...
const PassengerDashboard = () => {
  ...
  // Consolidate multiple O(N) and O(N log N) passes into a single O(N) useMemo block
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    let activeBookingsCount = 0;
    let kmTotal = 0;
    let closestJourneyTime = Infinity;
    const now = Date.now();
    ...
    for (let i = 0; i < safeTickets.length; i++) {
      ...
      // Calculate time for next journey finding minimum instead of sorting
      const time = Date.parse(t.date + (t.departureTime ? " " + t.departureTime : "")) || Date.parse(t.date);
      if (!isNaN(time) && time >= now && time < closestJourneyTime) {
        closestJourneyTime = time;
      }
    }

    return {
      activeBookings: activeBookingsCount,
      hasBookings: safeTickets.length > 0,
      latestPnr: safeTickets.length > 0 ? (safeTickets[0]?.pnr || "") : "",
      nextJourney: closestJourneyTime !== Infinity ? new Date(closestJourneyTime).toLocaleString() : "No upcoming",
