...
// Hoist static lookup map to avoid O(N) reallocation on every render
...
const PassengerDashboard = () => {
  ...
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    const hasBookings = safeTickets.length > 0;
    const latestPnr = hasBookings ? (safeTickets[0]?.pnr || "") : "";

    let activeBookings = 0;
    let kmTotal = 0;
    let nextJourneyTime = Infinity;
    const now = Date.now();
    ...
    for (let i = 0; i < safeTickets.length; i++) {
      ...
      // Find next journey min-time (O(N) instead of O(N log N) sort)
      const time = Date.parse(t.date + (t.departureTime ? " " + t.departureTime : "")) || Date.parse(t.date);
      if (!isNaN(time) && time >= now && time < nextJourneyTime) {
        nextJourneyTime = time;
      }
    }

    const nextJourney = nextJourneyTime !== Infinity ? new Date(nextJourneyTime).toLocaleString() : "No upcoming";
    const milesTraveled = Math.round(kmTotal * 0.621371);

    return { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled };
  }, [tickets]);
