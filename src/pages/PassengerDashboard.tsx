...
const PassengerDashboard = () => {
  ...
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    let active = 0;
    let kmTotal = 0;
    let nearestJourneyTime = Infinity;
    const now = Date.now();
    ...
    for (const t of safeTickets) {
      ...
      if (!isNaN(parsedTime) && parsedTime >= now && parsedTime < nearestJourneyTime) {
        nearestJourneyTime = parsedTime;
      }
    }

    const has = safeTickets.length > 0;
    const pnr = has ? (safeTickets[0]?.pnr || "") : "";
    const nextJ = nearestJourneyTime !== Infinity ? new Date(nearestJourneyTime).toLocaleString() : "No upcoming";
