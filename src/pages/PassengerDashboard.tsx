...
const PassengerDashboard = () => {
  ...
  const metrics = useMemo(() => {
    ...
    for (let i = 0; i < safeTickets.length; i++) {
      ...
      if (!isNaN(tTime) && tTime >= Date.now()) {
        if (tTime < nearestTime) {
          nearestTime = tTime;
        }
      }
    }
    ...
    const nextJourney = nearestTime !== Infinity ? new Date(nearestTime).toLocaleString() : "No upcoming";
    const milesTraveled = Math.round(kmTotal * 0.621371);

    return { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled };
  }, [tickets]);

  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = metrics;
