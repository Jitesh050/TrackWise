...
// Precompute static station lookup outside component to prevent reallocation per render
...
const PassengerDashboard = () => {
  ...
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = React.useMemo(() => {
    ...
    return {
      ...
      hasBookings: list.length > 0,
      latestPnr: list.length > 0 ? (list[0]?.pnr || "") : "",
      nextJourney: minTime === Infinity ? "No upcoming" : new Date(minTime).toLocaleString(),
