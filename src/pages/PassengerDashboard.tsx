...
// Pre-compute station lookup once
...
const PassengerDashboard = () => {
  ...
  // Optimize: single pass for all derived ticket metrics
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    let active = 0;
    let kmTotal = 0;
    let upcomingTime = Infinity;
    ...
    (tickets || []).forEach(t => {
      // Metrics for confirmed tickets
      if (t.status === "Confirmed") {
        active++;
        const fromCode = String(t.from || '').toUpperCase();
        const toCode = String(t.to || '').toUpperCase();
        const from = STATIONS_BY_CODE.get(fromCode);
        const to = STATIONS_BY_CODE.get(toCode);
        if (from && to) {
      ...
      if (!isNaN(time) && time >= now && time < upcomingTime) {
        upcomingTime = time;
      }
    });
    ...
    return {
      ...
      hasBookings,
      latestPnr,
      nextJourney: upcomingTime === Infinity ? "No upcoming" : new Date(upcomingTime).toLocaleString(),
      milesTraveled: Math.round(kmTotal * 0.621371), // convert km to miles
    };
  }, [tickets]);
