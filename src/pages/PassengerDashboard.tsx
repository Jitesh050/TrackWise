...
// Build quick lookup of station by code once
...
const PassengerDashboard = () => {
  ...
  // Optimization: Consolidate multiple iterative passes over tickets into a single useMemo block
  // Avoids redundant O(N) array traversals and O(N log N) sorting on every render
  const {
    activeBookings,
    hasBookings,
    latestPnr,
    ...
    for (const t of ticketList) {
      ...
      if (!isNaN(time) && time >= now && time < minFutureTime) {
        minFutureTime = time;
      }
    }

    const has = ticketList.length > 0;
    const pnr = has ? (ticketList[0]?.pnr || "") : "";
    const nextJ = minFutureTime !== Infinity ? new Date(minFutureTime).toLocaleString() : "No upcoming";
    const miles = Math.round(kmTotal * 0.621371);
