...
// Hoisted outside the component to prevent O(S) Map reallocation on every render
...
const PassengerDashboard = () => {
  ...
  // ⚡ Bolt: Consolidated multiple array passes (filter, map, reduce, sort) into a single O(N) pass
  // replacing the expensive O(N log N) date sort with an O(N) minimum-finding algorithm for nextJourney.
  const { activeBookings, hasBookings, latestPnr, nextJourney, milesTraveled } = useMemo(() => {
    ...
    return {
      ...
      nextJourney: minFutureTime !== Infinity ? new Date(minFutureTime).toLocaleString() : "No upcoming",
