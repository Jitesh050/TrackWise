...
const PassengerDashboard = () => {
  ...
  // ⚡ Bolt: Consolidated active bookings, next journey, and miles traveled calculation
  // into a single O(N) useMemo pass over the tickets array to avoid multiple iterations per render.
  // ⚡ Bolt: Hoisted static STATIONS_BY_CODE Map outside the component to prevent O(N) reallocation per render.
  const { activeBookings, nextJourney, milesTraveled } = useMemo(() => {
    ...
    tickets.forEach(t => {
      ...
      if (!isNaN(time) && time >= now && time < minUpcomingTime) {
        minUpcomingTime = time;
      }
    });

    return {
      activeBookings: activeCount,
      nextJourney: minUpcomingTime === Infinity ? "No upcoming" : new Date(minUpcomingTime).toLocaleString(),
