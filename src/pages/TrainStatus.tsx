...
const TrainStatus = () => {
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Use a single-pass reduce with early returns
    // to avoid mapping the entire array when only a subset is displayed.
    // Reduces O(N)*3 + array allocations down to O(N) operations.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const results = (trains || []).reduce((acc: any[], t: any) => {
