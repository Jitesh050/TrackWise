...
const TrainStatus = () => {
  ...
  // ⚡ Bolt: Replaced chained .map().filter().filter() with a single useMemo and reduce pass
  // to avoid redundant array allocations and iterations.
  const filteredTrains = useMemo(() => {
    const query = searchQuery.toLowerCase();

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (trains || []).reduce<any[]>((acc, t) => {
      const status = mapStatusToCard(t.status);

      // Early return if it doesn't match the tab
      ...
      // Early return if it doesn't match the search query
      if (
        query &&
