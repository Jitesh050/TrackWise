...
const TicketManagement = () => {
  ...
  const { filteredTickets, counts } = useMemo(() => {
    const lowerSearchTerm = searchTerm.toLowerCase();

    return tickets.reduce((acc, ticket) => {
      // Update counts
      if (ticket.status === "Confirmed") acc.counts.confirmed++;
      else if (ticket.status === "Waiting") acc.counts.waiting++;
      else if (ticket.status === "Cancelled") acc.counts.cancelled++;
