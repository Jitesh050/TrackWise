...
const TicketManagement = () => {
  ...
  // Consolidated 4 separate O(N) array passes (1 filter, 3 count) into a single
  // O(N) reduce pass wrapped in useMemo. We also hoisted the lowercasing of
  // the searchTerm out of the loop.
  // Impact: Reduces operations from ~4N to exactly N per render and avoids
  // recalculating entirely unless tickets, search, or filter changes.
  const { filteredTickets, stats } = useMemo(() => {
    ...
    return tickets.reduce(
      (acc, ticket) => {
        ...
        // Filter
        const matchesSearch =
          ticket.passengerName.toLowerCase().includes(sTerm) ||
          ticket.pnr.toLowerCase().includes(sTerm) ||
          ticket.trainNumber.toLowerCase().includes(sTerm);
      ...
      { filteredTickets: [] as TicketRecord[], stats: { confirmed: 0, waiting: 0, cancelled: 0, total: 0 } }
    );
