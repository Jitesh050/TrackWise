...
const TicketManagement = () => {
  ...
  const { ticketStats, filteredTickets } = useMemo(() => {
    const stats = { confirmed: 0, waiting: 0, cancelled: 0 };
    const lowerSearchTerm = searchTerm.toLowerCase();

    const filtered = tickets.filter(ticket => {
      // Calculate stats (independent of filter)
      if (ticket.status === "Confirmed") stats.confirmed++;
      else if (ticket.status === "Waiting") stats.waiting++;
      else if (ticket.status === "Cancelled") stats.cancelled++;
    ...
    return { ticketStats: stats, filteredTickets: filtered };
