...
const TicketManagement = () => {
  ...
  // ⚡ Bolt Optimization: Memoize aggregate stats into a single O(N) pass
  // Avoids 3 separate O(N) .filter().length operations on every render
  ...
  // ⚡ Bolt Optimization: Memoize filtering and hoist toLowerCase()
  // Avoids O(N) redundant string conversions during typing/re-renders
  const filteredTickets = useMemo(() => {
    const lowerSearchTerm = searchTerm.toLowerCase();

    return tickets.filter(ticket => {
      const matchesSearch =
        ticket.passengerName.toLowerCase().includes(lowerSearchTerm) ||
