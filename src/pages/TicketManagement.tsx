...
const TicketManagement = () => {
  ...
  // ⚡ Bolt Optimization: Consolidate multiple O(N) filters and map over tickets into a single pass
    ...
    return tickets.reduce(
      (acc, ticket) => {
        ...
        if (matchesSearch && matchesFilter) {
          acc.filteredList.push(ticket);
        }

        return acc;
      },
      {
        filteredList: [] as TicketRecord[],
        stats: { confirmed: 0, waiting: 0, cancelled: 0 }
      }
    );
  }, [tickets, searchTerm, filterStatus]);

  const { filteredList: filteredTickets, stats } = ticketData;
