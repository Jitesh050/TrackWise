...
const TicketManagement = () => {
  ...
  // ⚡ Bolt Optimization: Memoize the filtered list and hoist the lowercase conversion
  // to prevent O(N) string operations and redundant filtering on every render.
  const filteredTickets = useMemo(() => {
    const term = searchTerm.toLowerCase();
    return tickets.filter(ticket => {
      const matchesSearch = ticket.passengerName.toLowerCase().includes(term) ||
  ...
  // ⚡ Bolt Optimization: Consolidate multiple O(N) array `.filter().length` passes
  // into a single O(N) `.reduce()` pass and wrap in `useMemo` to prevent recalculation.
  const stats = useMemo(() => {
