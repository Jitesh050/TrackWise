...
const UserManagement = () => {
  ...
  const { filteredUsers, stats } = useMemo(() => {
    ...
    const result = {
      filteredUsers: [] as typeof STATIC_USERS,
      stats: {
    ...
    for (const user of STATIC_USERS) {
      // Calculate stats in the same pass
      if (user.role === "admin") result.stats.admins++;
      if (user.status === "active") result.stats.active++;
      if (user.role === "user") result.stats.regular++;
      ...
      if (matchesSearch && matchesFilter) {
        result.filteredUsers.push(user);
      }
    }
